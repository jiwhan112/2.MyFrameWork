...
HRESULT CMainApp::Render()
{
	...
	// 백버퍼 / 깊이버퍼 클리어
	m_pGameInstance->Clear_BackBuffer_View(_float4(0.7f, 0.7f, 0.7f,1));
...
HRESULT CMainApp::Ready_Prototype_Components_Shader()
{
	...
	FAILED_CHECK(m_pGameInstance->Add_Prototype(E_LEVEL::LEVEL_STATIC, TAGCOM(COMPONENT_SHADER_PARTICLE3D),
		CShader::Create(m_pDevice, m_pDeviceContext, TEXT("../Bin/ShaderFiles/Shader_VtxParticle.hlsl"),
			VTXMODEL_DECLARATION::Elements, VTXMODEL_DECLARATION::iNumElements)));
	//FAILED_CHECK(m_pGameInstance->Add_Prototype(E_LEVEL::LEVEL_STATIC, TAGCOM(COMPONENT_SHADER_UI),
	//	CShader::Create(m_pDevice, m_pDeviceContext, TEXT("../Bin/ShaderFiles/Shader_VtxTex_UI.hlsl"),
	//		VTXTEX_DECLARATION::Elements, VTXTEX_DECLARATION::iNumElements)));
